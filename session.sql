SELECT af.distinct_id,
 ...
 af.new_session_flag, af.page_duration, af.session_id, CONCAT(af.distinct_id, '_', CAST(af.time - SUM(af.datetime_flag) OVER(PARTITION BY af.distinct_id, af.session_id
ORDER BY  af.time) AS VARCHAR), '_', CAST(af.session_id AS VARCHAR)) AS session_id_full, CAST(from_unixtime((af.time - SUM(af.datetime_flag) OVER(PARTITION BY af.distinct_id, af.session_id
...
FROM 
    ...
    FROM 
        ...
        ORDER BY  time) AS rn, -- 每個人的流水號 (好像可以拿掉?)
         -- [lag series]
         -- LAG(time, 1) OVER(PARTITION BY distinct_id
        ORDER BY  time) AS lag_time, 
         -- > （可註解) 往後挪一筆, 計算 Session 用
         -- COALESCE(time - LAG(time, 1) OVER(PARTITION BY distinct_id
        ORDER BY  time),0) AS lag_diff_time, 
...
ORDER BY  time),0)) AS datetime_flag, -- > For 標註 Session 起始點用
 -- [lead series]
 -- LEAD(time, 1) OVER(PARTITION BY distinct_id
ORDER BY  time) AS lead_time, 
 -- > （可註解) 往前挪一筆, 計算 停留時間 用
 -- COALESCE(LEAD(time,1) OVER(PARTITION BY distinct_id
ORDER BY  time) -time ,0) AS lead_diff_time, 
...
WHERE partition_0 = '2020'
        ...
        AND mp_event_name NOT IN ('SysChk_distinct_id', '$campaign_delivery', '$campaign_received') ) AS aa ) AS a0 ) AS af 
